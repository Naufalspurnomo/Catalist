...
import { FiRefreshCw, FiAlertTriangle, FiCheck, FiUserPlus, FiUsers } from 'react-icons/fi';
...
const Users = () => {
  ...
  const handleToggleStatus = async (userId, newStatus) => {
    try {
      ...
      setSuccess(`Pengguna berhasil ${newStatus ? 'diaktifkan' : 'dinonaktifkan'}`);
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        <div 
          className={`bg-white rounded-lg shadow-sm p-4 cursor-pointer border-2 ${
            roleFilter === 'all' && statusFilter === 'all' ? 'border-primary' : 'border-transparent'
          }`}
          onClick={() => { handleRoleChange('all'); handleStatusChange('all'); }}
        >
          <div className="flex items-center">
        ...
        <div 
          className={`bg-white rounded-lg shadow-sm p-4 cursor-pointer border-2 ${
            statusFilter === 'inactive' ? 'border-primary' : 'border-transparent'
          }`}
